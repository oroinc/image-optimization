...
Feature: Image processor settings admin configuration
  ...
  Scenario: Use default jpeg and png quality
    ...
    And I should not see "Modification of the default value may cause temporary storefront slow-down until all product images are resized. The changes on the product listing page will not be applied immediately and will require manual start of the search re-index. Make sure that the harddrive has at least 50% space available as the resized images will be stored alongside the existing ones." flash message
  ...
  Scenario: Update jpeg and png quality
    ...
    And submit form
    Then I should see "Configuration saved" flash message
    And should see "Modification of the default value may cause temporary storefront slow-down until all product images are resized. The changes on the product listing page will not be applied immediately and will require manual start of the search re-index. Make sure that the harddrive has at least 50% space available as the resized images will be stored alongside the existing ones." flash message
    # The message does not exist because the configuration not changed
    When I reload the page
    And submit form
    Then I should see "Configuration saved" flash message
    And should not see "Modification of the default value may cause temporary storefront slow-down until all product images are resized. The changes on the product listing page will not be applied immediately and will require manual start of the search re-index. Make sure that the harddrive has at least 50% space available as the resized images will be stored alongside the existing ones." flash message
